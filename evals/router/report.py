...
def write_report(metrics: dict[str, Any], records: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        ...
        f"Embedding provider: `{metadata.get('embedding_provider') or ''}`",
        f"Embedding model: `{metadata.get('embedding_model') or ''}`",
        f"Embedding local files only: `{metadata.get('embedding_local_files_only')}`",
        ...
        f"LLM escalated examples: {metrics['llm_escalated_examples']}",
